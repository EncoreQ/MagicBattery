...
蓝牙(VID 0x004C, 充电中,  满电 100%): report 0x90 = 90 03 64
蓝牙(VID 0x004C, 拔线,    满电 100%): report 0x90 = 90 00 64
                                                ↑b1 ↑b2
```

**补录(2026-06-08 同台设备,满电):** 充满后经**蓝牙接口**读到 `90 03 64`,
拔线后立即再读 `90 00 64`(仅 byte[1] 从 0x03 翻 0x00,byte[2] 不变)。三个新结论:

1. **byte[2] 高值仍直读**:0x64=100,印证 0–100 无缩放在满量程依然成立。
2. **byte[1] 充电标志独立于 VID/传输**:充电样本走蓝牙(VID 004C)却仍为 0x03,
   说明 byte[1] 反映的是「设备是否接入外部供电/充电」,**与读它的 HID 接口是 USB 还是蓝牙无关**。
   故 `IsCharging` 必须读 byte[1] 真值,**不能**用「连接类型==USB」近似(下方影响清单已据此修正)。
3. **byte[1] 即纯充电标志**:满电、电量不变的前提下拔线,byte[1] 0x03→0x00,
   排除了「byte[1] 含电量高位」等猜测 —— 它只随充电状态翻转。
...
| 连接判别 | 设备 VID:`0x05AC`=USB、`0x004C`=蓝牙;PID 均 `0x0265`。**插到本机 USB 时蓝牙的 004C HID 接口会消失**(设备数据切到 USB)。但从**其它电源**(充电器/别的主机)充电时,本机仍只见 004C 蓝牙接口,此时 byte[1] 照样为充电态(见上「补录」满电样本) |
