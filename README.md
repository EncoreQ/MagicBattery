...
MVP（Phase 2）已完成：托盘图标 + 悬停电量 + USB/蓝牙自动选路。电量协议已用真机校准。
...
| Phase 3 | Toast 低电量告警 + 多设备 + 配置持久化 | 计划中 |

## 功能（Phase 2）

- 托盘图标按电量分 **5 档**变色（>75 / 50 / 25 / 10 / <10），图标上直接显示百分比数字，充电时叠加闪电。
- 悬停 tooltip：百分比 + 连接方式（USB/蓝牙）+ 最后更新时间。
- 右键菜单：立即刷新 / 开机自启 / 退出。
- 默认 15 分钟轮询；监听设备插拔（`WM_DEVICECHANGE`），USB↔蓝牙切换数秒内生效。
- 开机自启走 HKCU `Run` 键，**无需管理员**。
...
- 校准样本（`tests/fixtures/report-0x90/`）：`90 00 02`（蓝牙 2%）、`90 03 03`（USB 充电 3%）、`90 03 64` / `90 00 64`（满电 充电/拔线）。
