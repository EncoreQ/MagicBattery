...
- ~~中/高电量点~~ 已补:满电 100% = `90 03 64`(byte[2] 高值直读确认)。
- 设备睡眠/刚唤醒的怪值(spec §8 U5)。
- `byte[1]` 充电标志的逐位精确含义(目前只知 0x00=未充电、0x03=充电;0x03 在「USB 接口读」和「充电中经蓝牙接口读」下均出现,与 VID 无关)。
- 真正拔线满电(byte[1] 应回 0x00、byte[2]=100)还没采到 —— 当前满电样本是充电态。
